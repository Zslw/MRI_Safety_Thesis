...
In Figure \ref{fig:m0mcost}, the dipole of magnitude $m$, experiences a torque that follows Equation \ref{eq:torque}. From the geometric projection of the dipole moment onto the magnetic field direction (eq. \ref{eq:mprojection}), the torque experienced by the dipole is	
\begin{equation*}
...
\begin{subequations}
	\begin{equation}
		\tau(\theta) = \tau_{max} \sin(2\theta), \quad \text{or}
...
\noindent where $\tau_{max}$ is the maximum torque measured or obtained by equation \ref{eq:maxtorque}, thus the maximum induced torque is comparable to the product of the device's longest dimension ($l$) and its weight ($mg$). Under the ASTM gravity-based acceptability criterion, a device is considered acceptable when $c_\tau < 1$.