...
The experimental frame was constructed entirely from \gls{pvc} components (Figure \ref{fig:schematic_a}), including a vertical post of about 1.5 meters and a rigid base for stabilization. Attached to the pole a set of rings were attached to the pole with a nylon screw to held positions. The lower one features a slit, where a custom made Kydex ruler sits held by smaller nylon screws. This ruler is interchangeable. The upper ring shows a rubber clamp from which a string passes. Then the lenght of suspentson $L= 1$ meter was set as the distance between these two rings, more specifically from the last contact point of the string to the rubber on the top ring to the sample hanging free at ruler's height as shown in Figure~\ref{fig:schematic_c}. 

 The sample—a \textbf{solid ferromagnetic cylinder} (length = 6 mm, diameter = 3 mm, mass = 0.3648 g, density = 8.6 g/cm$^3$)—was suspended by a \textbf{955 mm-long sewing thread}, tied via a simple knot at its center. For clinical applications, a \textbf{0.25 mm nylon monofilament} is recommended for improved reproducibility and safety. A Monofilament Nylon thread has low stretch,  complies with MR-safe properties (Non-metallic, Non-conductive, Non-magnetic), and reduces variability introduced by torsional drift or thread elasticity  \cite{stoianovici2024, astmF2052}. The bottom of the sample was free to swing, forming a pendulum under gravity. A horizontal ruler, fixed along the Z-axis, served as the visual reference to quantify lateral displacements as shown in Figure \ref{fig:schematic_b} 
...
\begin{figure}[H]
	...
	\begin{subfigure}[b]{0.48\textwidth}
		...
		\caption{Real-life implementation of the apparatus in the laboratory setting.}
	...
	\begin{subfigure}[b]{0.55\textwidth}
		\centering
		\includegraphics[width=\textwidth]{Assests/Picture2.jpg}
		\caption{Close-up of the epicardial lead fragment suspended by monofilament thread over the ruler.}
		\label{fig:schematic_c}
	\end{subfigure}
	
	\caption{Experimental pendulum-based apparatus constructed from PVC for MRI translational force measurements: (a) CAD model, (b) laboratory setup, and (c) displacement measurement close-up.}
	\label{fig:schematic}
\end{figure}
...
A summary schematic of the apparatus and frame are included in Figures \ref{fig:schematic} and \ref{fig:leadHolder}, with additional engineering drawings available in Appendix \ref{app:A1} and \ref{app:A2}
