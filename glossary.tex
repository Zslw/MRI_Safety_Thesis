\makenoidxglossaries

\newacronym{cdr}{CDR}{Creighton Digital Repository}
\newacronym{bergan}{CUMC--Bergan Mercy}{CHI Health Creighton University Medical Center--Bergan Mercy}

\newacronym{fr}{FR}{Force Ratio}
\newacronym[
longplural={Materials Under Test}
]{mut}{MUT}{Material Under Test}

\newacronym{daq}{DAQ}{Data Acquisition}
\newacronym{hrs}{HRS}{Heart Rhythm Society}
\newacronym{esc}{ESC}{European Society of Cardiology}
\newacronym{cms}{CMS}{Centers for Medicare \& Medicaid Services}
\newacronym{cied}{CIED}{cardiac implantable electronic device}
\newacronym{icd}{ICD}{implantable cardioverter-defibrillator}
\newacronym{crt}{CRT}{cardiac resynchronization therapy}
\newacronym{ecg}{ECG}{electrocardigram}

\newacronym{aimd}{AIMD}{active implantable medical device}
\newacronym{kcl}{KCL}{Kirchhoff’s Current Law}

\newacronym{ssg}{SSG}{Slice Select Gradient }
\newacronym{peg}{PEG}{Phase Encoding Gradient}
\newacronym{feg}{FEG}{Frequency Encoding Gradient}

\newacronym{pns}{PNS}{peripheral nerve stimulation}

\newacronym{sar}{SAR}{Specific Absorption Rate}
\newacronym{csv}{CSV}{Comma Separated Values}

\newacronym{rf}{RF}{radiofrequency}
\newacronym{dac}{DAC}{data acquisition card}
\newacronym{pd}{PD}{Proton Density}
\newacronym{pla}{PLA}{Polylactic Acid}
\newacronym{pvc}{PVC}{Polyvinyl Chloride}
\newacronym{lvi}{VI}{LabVIEW Virtual Instrument}
\newacronym{mri}{MRI}{Magnetic Resonance Imaging}

\definecolor{steelblue}{rgb}{0.27, 0.51, 0.71}
\definecolor{crimson}{rgb}{0.86, 0.08, 0.24}
\definecolor{darkslategray}{rgb}{0.18, 0.31, 0.31}

\hypersetup{colorlinks   = true, %Colours links instead of ugly boxes
	urlcolor     = steelblue, %Colour for external hyperlinks
	linkcolor    = darkslategray, %Colour of internal links
	citecolor   = crimson %Colour of citations
    }